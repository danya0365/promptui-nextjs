import {
  AiModel,
  IShowcaseLivePreviewRepository,
  ShowcaseLivePreview,
} from "@/src/application/repositories/IShowcaseLivePreviewRepository";

const LIVE_PREVIEWS: ShowcaseLivePreview[] = [
  {
    id: "lp-001",
    showcaseId: "showcase-001",
    aiModel: "claude-4-sonnet",
    isActive: true,
    createdAt: "2025-06-15T10:30:00.000Z",
  },
  {
    id: "lp-002",
    showcaseId: "showcase-001",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-15T11:00:00.000Z",
  },
  {
    id: "lp-003",
    showcaseId: "showcase-002",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-16T09:00:00.000Z",
  },
  {
    id: "lp-004",
    showcaseId: "showcase-008",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-17T14:00:00.000Z",
  },
  {
    id: "lp-005",
    showcaseId: "showcase-003",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T09:00:00.000Z",
  },
  {
    id: "lp-006",
    showcaseId: "showcase-004",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T10:00:00.000Z",
  },
  {
    id: "lp-007",
    showcaseId: "showcase-005",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T11:00:00.000Z",
  },
  {
    id: "lp-008",
    showcaseId: "showcase-006",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T12:00:00.000Z",
  },
  {
    id: "lp-009",
    showcaseId: "showcase-007",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T13:00:00.000Z",
  },
  {
    id: "lp-010",
    showcaseId: "showcase-009",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T14:00:00.000Z",
  },
  {
    id: "lp-011",
    showcaseId: "showcase-010",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T15:00:00.000Z",
  },
  {
    id: "lp-012",
    showcaseId: "showcase-011",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T16:00:00.000Z",
  },
  {
    id: "lp-013",
    showcaseId: "showcase-012",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T17:00:00.000Z",
  },
  {
    id: "lp-014",
    showcaseId: "showcase-013",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T18:00:00.000Z",
  },
  {
    id: "lp-015",
    showcaseId: "showcase-014",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T19:00:00.000Z",
  },
  {
    id: "lp-016",
    showcaseId: "showcase-015",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T20:00:00.000Z",
  },
  {
    id: "lp-017",
    showcaseId: "showcase-016",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T21:00:00.000Z",
  },
  {
    id: "lp-018",
    showcaseId: "showcase-017",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T22:00:00.000Z",
  },
  {
    id: "lp-019",
    showcaseId: "showcase-018",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T23:00:00.000Z",
  },
  {
    id: "lp-020",
    showcaseId: "showcase-019",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-18T23:30:00.000Z",
  },
  {
    id: "lp-021",
    showcaseId: "showcase-020",
    aiModel: "gemini-3-pro",
    isActive: true,
    createdAt: "2025-06-19T00:00:00.000Z",
  },
  {
    id: "lp-022",
    showcaseId: "showcase-021",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T01:00:00.000Z",
  },
  {
    id: "lp-023",
    showcaseId: "showcase-022",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T02:00:00.000Z",
  },
  {
    id: "lp-024",
    showcaseId: "showcase-023",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T03:00:00.000Z",
  },
  {
    id: "lp-025",
    showcaseId: "showcase-024",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T04:00:00.000Z",
  },
  {
    id: "lp-026",
    showcaseId: "showcase-025",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T05:00:00.000Z",
  },
  {
    id: "lp-027",
    showcaseId: "showcase-026",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T06:00:00.000Z",
  },
  {
    id: "lp-028",
    showcaseId: "showcase-027",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T07:00:00.000Z",
  },
  {
    id: "lp-029",
    showcaseId: "showcase-028",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T08:00:00.000Z",
  },
  {
    id: "lp-030",
    showcaseId: "showcase-029",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T09:00:00.000Z",
  },
  {
    id: "lp-031",
    showcaseId: "showcase-030",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T10:00:00.000Z",
  },
  {
    id: "lp-032",
    showcaseId: "showcase-031",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T11:00:00.000Z",
  },
  {
    id: "lp-033",
    showcaseId: "showcase-032",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T12:00:00.000Z",
  },
  {
    id: "lp-034",
    showcaseId: "showcase-033",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T13:00:00.000Z",
  },
  {
    id: "lp-035",
    showcaseId: "showcase-034",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T14:00:00.000Z",
  },
  {
    id: "lp-036",
    showcaseId: "showcase-035",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T15:00:00.000Z",
  },
  {
    id: "lp-037",
    showcaseId: "showcase-036",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T16:00:00.000Z",
  },
  {
    id: "lp-038",
    showcaseId: "showcase-037",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T17:00:00.000Z",
  },
  {
    id: "lp-039",
    showcaseId: "showcase-038",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T18:00:00.000Z",
  },
  {
    id: "lp-040",
    showcaseId: "showcase-039",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T19:00:00.000Z",
  },
  {
    id: "lp-041",
    showcaseId: "showcase-040",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T20:00:00.000Z",
  },
  {
    id: "lp-042",
    showcaseId: "showcase-041",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T21:00:00.000Z",
  },
  {
    id: "lp-043",
    showcaseId: "showcase-042",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T22:00:00.000Z",
  },
  {
    id: "lp-044",
    showcaseId: "showcase-043",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-19T23:00:00.000Z",
  },
  {
    id: "lp-045",
    showcaseId: "showcase-044",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T00:00:00.000Z",
  },
  {
    id: "lp-046",
    showcaseId: "showcase-045",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T01:00:00.000Z",
  },
  {
    id: "lp-047",
    showcaseId: "showcase-046",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T02:00:00.000Z",
  },
  {
    id: "lp-048",
    showcaseId: "showcase-047",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T03:00:00.000Z",
  },
  {
    id: "lp-049",
    showcaseId: "showcase-048",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T04:00:00.000Z",
  },
  {
    id: "lp-050",
    showcaseId: "showcase-049",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T05:00:00.000Z",
  },
  {
    id: "lp-051",
    showcaseId: "showcase-050",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T06:00:00.000Z",
  },
  {
    id: "lp-052",
    showcaseId: "showcase-051",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T07:00:00.000Z",
  },
  {
    id: "lp-053",
    showcaseId: "showcase-052",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T08:00:00.000Z",
  },
  {
    id: "lp-054",
    showcaseId: "showcase-053",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T09:00:00.000Z",
  },
  {
    id: "lp-055",
    showcaseId: "showcase-054",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T10:00:00.000Z",
  },
  {
    id: "lp-056",
    showcaseId: "showcase-055",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T11:00:00.000Z",
  },
  {
    id: "lp-057",
    showcaseId: "showcase-056",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T12:00:00.000Z",
  },
  {
    id: "lp-058",
    showcaseId: "showcase-057",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T13:00:00.000Z",
  },
  {
    id: "lp-059",
    showcaseId: "showcase-058",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T14:00:00.000Z",
  },
  {
    id: "lp-060",
    showcaseId: "showcase-059",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T15:00:00.000Z",
  },
  {
    id: "lp-061",
    showcaseId: "showcase-060",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T16:00:00.000Z",
  },
  {
    id: "lp-062",
    showcaseId: "showcase-061",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T17:00:00.000Z",
  },
  {
    id: "lp-063",
    showcaseId: "showcase-062",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T18:00:00.000Z",
  },
  {
    id: "lp-064",
    showcaseId: "showcase-063",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T19:00:00.000Z",
  },
  {
    id: "lp-065",
    showcaseId: "showcase-064",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T20:00:00.000Z",
  },
  {
    id: "lp-066",
    showcaseId: "showcase-065",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T21:00:00.000Z",
  },
  {
    id: "lp-067",
    showcaseId: "showcase-066",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T22:00:00.000Z",
  },
  {
    id: "lp-068",
    showcaseId: "showcase-067",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-20T23:00:00.000Z",
  },
  {
    id: "lp-069",
    showcaseId: "showcase-068",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T00:00:00.000Z",
  },
  {
    id: "lp-070",
    showcaseId: "showcase-069",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T01:00:00.000Z",
  },
  {
    id: "lp-071",
    showcaseId: "showcase-070",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T02:00:00.000Z",
  },
  {
    id: "lp-072",
    showcaseId: "showcase-071",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T03:00:00.000Z",
  },
  {
    id: "lp-073",
    showcaseId: "showcase-072",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T04:00:00.000Z",
  },
  {
    id: "lp-074",
    showcaseId: "showcase-073",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T05:00:00.000Z",
  },
  {
    id: "lp-075",
    showcaseId: "showcase-074",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T06:00:00.000Z",
  },
  {
    id: "lp-076",
    showcaseId: "showcase-075",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T07:00:00.000Z",
  },
  {
    id: "lp-077",
    showcaseId: "showcase-076",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T08:00:00.000Z",
  },
  {
    id: "lp-078",
    showcaseId: "showcase-077",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T09:00:00.000Z",
  },
  {
    id: "lp-079",
    showcaseId: "showcase-078",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T10:00:00.000Z",
  },
  {
    id: "lp-080",
    showcaseId: "showcase-079",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T11:00:00.000Z",
  },
  {
    id: "lp-081",
    showcaseId: "showcase-080",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T12:00:00.000Z",
  },
  {
    id: "lp-082",
    showcaseId: "showcase-081",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T13:00:00.000Z",
  },
  {
    id: "lp-083",
    showcaseId: "showcase-082",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T14:00:00.000Z",
  },
  {
    id: "lp-084",
    showcaseId: "showcase-083",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T15:00:00.000Z",
  },
  {
    id: "lp-085",
    showcaseId: "showcase-084",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T16:00:00.000Z",
  },
  {
    id: "lp-086",
    showcaseId: "showcase-085",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T17:00:00.000Z",
  },
  {
    id: "lp-087",
    showcaseId: "showcase-086",
    aiModel: "gemini-3.1-pro",
    isActive: true,
    createdAt: "2025-06-21T18:00:00.000Z",
  },
  {
    id: "lp-088",
    showcaseId: "showcase-087",
    aiModel: "claude-4.6-opus",
    isActive: true,
    createdAt: "2025-06-21T19:00:00.000Z",
  },
  {
    id: "lp-089",
    showcaseId: "showcase-088",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T10:00:00.000Z",
  },
  {
    id: "lp-090",
    showcaseId: "showcase-089",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T11:00:00.000Z",
  },
  {
    id: "lp-091",
    showcaseId: "showcase-090",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T12:00:00.000Z",
  },
  {
    id: "lp-092",
    showcaseId: "showcase-091",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T13:00:00.000Z",
  },
  {
    id: "lp-093",
    showcaseId: "showcase-092",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T14:00:00.000Z",
  },
  {
    id: "lp-094",
    showcaseId: "showcase-093",
    aiModel: "claude-4.6-sonnet",
    isActive: true,
    createdAt: "2025-06-22T15:00:00.000Z",
  },
  {
    id: "lp-095",
    showcaseId: "showcase-093",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T15:00:00.000Z",
  },
  {
    id: "lp-096",
    showcaseId: "showcase-094",
    aiModel: "claude-4.6-sonnet",
    isActive: true,
    createdAt: "2025-06-22T16:00:00.000Z",
  },
  {
    id: "lp-097",
    showcaseId: "showcase-094",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T16:00:00.000Z",
  },
  {
    id: "lp-098",
    showcaseId: "showcase-095",
    aiModel: "claude-4.6-sonnet",
    isActive: true,
    createdAt: "2025-06-22T17:00:00.000Z",
  },
  {
    id: "lp-099",
    showcaseId: "showcase-095",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T17:00:00.000Z",
  },
  {
    id: "lp-100",
    showcaseId: "showcase-096",
    aiModel: "claude-4.6-sonnet",
    isActive: true,
    createdAt: "2025-06-22T18:00:00.000Z",
  },
  {
    id: "lp-101",
    showcaseId: "showcase-096",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T18:00:00.000Z",
  },
  {
    id: "lp-102",
    showcaseId: "showcase-097",
    aiModel: "claude-4.6-sonnet",
    isActive: true,
    createdAt: "2025-06-22T19:00:00.000Z",
  },
  {
    id: "lp-103",
    showcaseId: "showcase-097",
    aiModel: "gemini-3-flash",
    isActive: true,
    createdAt: "2025-06-22T19:00:00.000Z",
  },
  {
    id: "lp-104",
    showcaseId: "showcase-098",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T00:00:00.000Z",
  },
  {
    id: "lp-105",
    showcaseId: "showcase-099",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T20:00:00.000Z",
  },
  {
    id: "lp-106",
    showcaseId: "showcase-100",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T21:00:00.000Z",
  },
  {
    id: "lp-107",
    showcaseId: "showcase-101",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T21:15:00.000Z",
  },
  {
    id: "lp-108",
    showcaseId: "showcase-102",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T21:25:00.000Z",
  },
  {
    id: "lp-109",
    showcaseId: "showcase-103",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T21:30:00.000Z",
  },
  {
    id: "lp-110",
    showcaseId: "showcase-104",
    aiModel: "kimi-k2.6",
    isActive: true,
    createdAt: "2025-06-12T21:40:00.000Z",
  },
  {
    id: "lp-111",
    showcaseId: "showcase-105",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:00:00.000Z",
  },
  {
    id: "lp-112",
    showcaseId: "showcase-106",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:10:00.000Z",
  },
  {
    id: "lp-113",
    showcaseId: "showcase-107",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:20:00.000Z",
  },
  {
    id: "lp-114",
    showcaseId: "showcase-108",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:30:00.000Z",
  },
  {
    id: "lp-115",
    showcaseId: "showcase-109",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:40:00.000Z",
  },
  {
    id: "lp-116",
    showcaseId: "showcase-110",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T00:50:00.000Z",
  },
  {
    id: "lp-117",
    showcaseId: "showcase-111",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:00:00.000Z",
  },
  {
    id: "lp-118",
    showcaseId: "showcase-112",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:10:00.000Z",
  },
  {
    id: "lp-119",
    showcaseId: "showcase-113",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:20:00.000Z",
  },
  {
    id: "lp-120",
    showcaseId: "showcase-114",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:30:00.000Z",
  },
  {
    id: "lp-121",
    showcaseId: "showcase-115",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:40:00.000Z",
  },
  {
    id: "lp-122",
    showcaseId: "showcase-116",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T01:50:00.000Z",
  },
  {
    id: "lp-123",
    showcaseId: "showcase-117",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:00:00.000Z",
  },
  {
    id: "lp-124",
    showcaseId: "showcase-118",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:10:00.000Z",
  },
  {
    id: "lp-125",
    showcaseId: "showcase-119",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:20:00.000Z",
  },
  {
    id: "lp-126",
    showcaseId: "showcase-120",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:30:00.000Z",
  },
  {
    id: "lp-127",
    showcaseId: "showcase-121",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:40:00.000Z",
  },
  {
    id: "lp-128",
    showcaseId: "showcase-122",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T02:50:00.000Z",
  },
  {
    id: "lp-129",
    showcaseId: "showcase-123",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T03:00:00.000Z",
  },
  {
    id: "lp-130",
    showcaseId: "showcase-124",
    aiModel: "claude-4.8-opus",
    isActive: true,
    createdAt: "2026-06-13T03:10:00.000Z",
  },
];

export class StaticShowcaseLivePreviewRepository implements IShowcaseLivePreviewRepository {
  private previews: ShowcaseLivePreview[] = [...LIVE_PREVIEWS];

  async getByShowcaseId(showcaseId: string): Promise<ShowcaseLivePreview[]> {
    return this.previews.filter(
      (p) => p.showcaseId === showcaseId && p.isActive,
    );
  }

  async getByAiModel(model: AiModel): Promise<ShowcaseLivePreview[]> {
    if (model === ("all" as string)) return this.getAll();
    return this.previews.filter((p) => p.aiModel === model && p.isActive);
  }

  async getAll(): Promise<ShowcaseLivePreview[]> {
    return this.previews.filter((p) => p.isActive);
  }

  async getShowcaseIdsWithPreviews(): Promise<string[]> {
    const ids = new Set(
      this.previews.filter((p) => p.isActive).map((p) => p.showcaseId),
    );
    return Array.from(ids);
  }
}

export const staticShowcaseLivePreviewRepository =
  new StaticShowcaseLivePreviewRepository();
